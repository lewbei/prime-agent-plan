#!/usr/bin/env python3
"""EpiPlanBench-Smoke: Synthetic Epistemic Plan Verification Smoke Runner & Report Generator."""
...
def generate_markdown_report(results_by_arm: Dict[str, List[EpiPlanResult]], output_path: str) -> None:
    lines = [
    ...
    for arm_id, res_list in results_by_arm.items():
        ...
        lines.append(
            f"| `{arm_id}` | **{desc}** | **{success_rate:.1f}%** | **{false_pass_rate:.1f}%** | **{safety_score:.2f}** | {mean_latency:.1f} ms |"
        )
...
def main():
    print("=" * 70)
    print("STARTING EPIPLANBENCH-SMOKE SYNTHETIC EVALUATION")
    print(f"Total Tasks: {len(TASKS)} across 7 synthetic verification domains")
    print("=" * 70)

    runner = EpiPlanBenchRunner()
    results_by_arm = {}
    ...
    generate_markdown_report(results_by_arm, report_file)
