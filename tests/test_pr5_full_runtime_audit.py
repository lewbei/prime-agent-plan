...
def _legacy_plan_with_input() -> str:
    return (
        ...
        "## Tasks\n1. Read input. Inputs: input.txt. Output: out.txt.\n"
    )
...
async def test_judge_ensemble_does_not_reuse_prior_plan_version_votes(tmp_path, monkeypatch):
    ...
    entry = await plan_mode.judge_ensemble(
        session,
        "1. Second changed plan. Output: b.txt.\n",
        "stale-judge-regression",
        n=3,
        plans_dir=tmp_path,
    )
...
def test_malformed_probe_output_is_unknown_not_empirical_false():
    ...
    integer_probe = DiagnosticProbe(
        probe_id="integer",
        target_predicate="count",
        argv_pipeline=[["echo", "x"]],
        expected_output_parser="integer",
    )
    json_probe = DiagnosticProbe(
        probe_id="json",
        target_predicate="json",
        argv_pipeline=[["echo", "x"]],
        expected_output_parser="json",
    )
...
def test_saga_recovery_executes_compensation_and_observes_failure():
    ...
    report = manager.execute_saga_rollback(
        [step],
        PlanIR(plan_id="p", goal_description="g", actions=[action]),
        registry,
        EvidenceLedger(),
        session,
    )
...
def test_registry_rejects_undeclared_extra_parameters():
    ...
    action = ActionIR(
        ...
        parameters={"x": 1, "undeclared": "surprise"},
        provenance=_prov(),
    )
...
def test_recovery_decision_does_not_mutate_input_evidence():
    evidence = DriftEvidence(task="t", step=4, K=2, why="x")
