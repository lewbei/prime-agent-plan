...
Inspired by LLM-as-a-Verifier (arXiv:2607.05391). Probabilistic scores rank
candidate plans but never create empirical facts or certify execution.
Certification remains the responsibility of Prime's deterministic validator,
authorization boundary, and runtime empirical witnesses.

Default behavior is same-model Gemini 3.7 Flash -> Gemini 3.7 Flash. Callers do
not select a separate self-verification mode; ``PlanSelfVerifier.select`` uses
that configuration unless explicitly overridden.
"""
...
class PlanSelfVerifier:
    """Closed-world prefilter -> same-model rank -> deterministic revalidation."""
    ...
    def select_same_model(
        ...
        *,
        ...
        """Deprecated compatibility alias; normal ``select`` already uses same-model defaults."""
        return self.select(
            candidates,
